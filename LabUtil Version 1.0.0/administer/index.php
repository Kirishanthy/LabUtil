	...
	while($row_mac = mysqli_fetch_array($result_mac))
	{
		...
		if($row_check > 0){
			...
			//echo '<tr><td ALIGN="CENTER"> '.$mac.'</td><td ALIGN="CENTER">'.gmdate("H:i:s", $c).'</td><td ALIGN="CENTER">Insert '.date('Y-m-d H:i:s').'</td></tr>';
                                    ...
                                    <aside class="bg-success dk  clearfix lter r-r text-right v-middle">
                            ...
                            <section class="col-lg-4 scrollable animated fadeInRight">
                                ...
                                <section class="panel bg-gradient no-borders">
                                    <header class="header text-center bg-warning dk"><p class="h4"><i class="icon-windows"></i><span class="h4"><?PHP echo $lang['INDEX_ONLINE_HEADER'];?></span></p></header>
                                    ...
                                    <footer class="panel-footer bg-black dk">
                                        <div class="row">
                                            <?PHP
                                            ...
                                            while ($row_option = mysqli_fetch_array($result_option)) {
                                                ...
                                                $number_of_online = mysqli_num_rows($result);
                                                echo ' <div class="col-xs-3"><span class="label bg-dark h4 block">' . $grop . '<span class="block">' . $number_of_online . '</span></span> </div>';
                                            }
                                            ?>
                                ...
                                <section class="panel no-borders bg-gradient bg-success dker">
                                    <header class="header text-center bg-warning dk"><p class="h4 text-white"><i class="icon-desktop"></i><?PHP echo $lang['INDEX_CLIENT_HEADER'];?> </p></header>
                                    <ul class="list-group list-group-flush no-borders">
                                        ...
                                        while ($row = mysqli_fetch_array($result_option)) {
                                            ...
                                            echo ' <li class="list-group-item h5 bg-black no-borders"> <span class="label bg-dark pull-right">' . $rows . '</span> <span class="label bg-dark">' . number_format($number++, 00) . '</span> <span class="label bg-dark">' . $row['name'] . '</span> </li>';
                                        }
                                        ?>
                                   </ul>
                               </section>
                                ...
                                <section class="panel no-borders bg-gradient bg-success  dker">
                                    <header class="header text-center bg-warning dk"><p class="h4"><i class="icon-male"></i><?PHP echo $lang['INDEX_USER_HEADER'];?></p></header>
                                    <ul class="list-group list-group-flush  bg-success alt no-borders ">
                                        <?php
                                        $number = 01;
